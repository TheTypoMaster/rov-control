...
long MS5803::readAdcPreassure(){
			 ...
			 sendCommand(MS5803_D1_512); // send acquire pressure
			 delay(10);
			 sendCommand(MS5803_AdcRead); // send Read adc.
			 Wire.requestFrom(MS5803_I2C_ADDRESS , 3, true);
...
long MS5803::readAdcTemperature(){
	// reads and corrects the temperature 
	
	sendCommand(MS5803_D2_512); // send acquire temperature
	delay(10);
...
int MS5803::getDepth(){
	// calculate depth from preasure and temp data
	 // get adcPreasure and temp
	 AdcPressure = readAdcPreassure();
	 AdcTemperature = readAdcTemperature();
